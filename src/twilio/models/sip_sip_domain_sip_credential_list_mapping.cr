...
module Twilio
  class SipSipDomainSipCredentialListMapping
    ...
    def list_invalid_properties
      ...
      if _domain_sid = @domain_sid
        if _domain_sid.to_s.size > 34
    ...
    def valid?
      ...
      if _domain_sid = @domain_sid
        return false if _domain_sid.to_s.size > 34
        return false if _domain_sid.to_s.size < 34
        return false if _domain_sid !~ /^SD[0-9a-fA-F]{32}$/
      end
      if _sid = @sid
        return false if _sid.to_s.size > 34
        return false if _sid.to_s.size < 34
        return false if _sid !~ /^CL[0-9a-fA-F]{32}$/
      end
